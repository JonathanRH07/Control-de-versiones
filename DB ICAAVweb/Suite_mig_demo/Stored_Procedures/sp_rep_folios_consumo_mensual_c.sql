DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `sp_rep_folios_consumo_mensual_c`(
...
BEGIN
/*
	@nombre:		sp_rep_folios_consumo_mensual_c
	@fecha:			22/05/2019
...
*/
	...
	DROP TEMPORARY TABLE IF EXISTS tmp_consumo_mensual;
	...
	SET @query = CONCAT('
						...
						FROM (
							...
							AND DATE_FORMAT(fecha, ''%Y'') = ''',pr_anio,''') a
