DELIMITER $$
CREATE DEFINER=`suite_deve`@`%` PROCEDURE `sp_cat_serie_i`(
...
*/
	...
	BEGIN
		SET pr_message = 'SERIE.MESSAGE_ERROR_CREATE_SERIES';
    ...
    /* Obtener si existe una serie activa de anticipos duplicada */
    SELECT
		COUNT(*)
	INTO
		lo_count
	...
	# INICIO
	IF pr_cve_tipo_serie != 'FNCA' THEN

		/* Valida si ya existe la clave del cliente */
		CALL sp_help_get_row_count_params(
				'ic_cat_tr_serie',
				pr_id_grupo_empresa,
				CONCAT(' cve_serie =  "', pr_cve_serie,'"'),@has_relations_with_series,pr_message);


		IF @has_relations_with_series > 0 THEN

			SET @error_code = 'CVE_DUPLICATE';
			SET pr_message = 'SERIE.DUPLICATED_CODE'; /*CONCAT('{"error": "4002", "code": "', @error_code, '", "count": ',(@has_relations_with_series),'}');*/
			SET pr_affect_rows = 0;
			ROLLBACK;
...
*/
    ...
    # INICIO
	ELSEIF pr_cve_tipo_serie = 'FNCA' AND lo_count > 0 THEN
		/* Serie activa de anticipos duplicada en la misma sucursal */
		SET pr_message = ('SERIE.MESSAGE_ERROR_DUPLICATE_FNC_ANT');

        SELECT FOUND_ROWS() INTO pr_affect_rows FROM DUAL;
	# FIN
