DELIMITER $$
CREATE DEFINER=`suite_deve`@`%` PROCEDURE `sp_fac_inventario_boletos_u`(
...
BEGIN
	/*
		@nombre:		sp_fac_inventario_boletos_u
		@fecha:			28/08/2017
	...
	*/
	...
	BEGIN
		SET pr_message = 'ERROR store sp_fac_inventario_boletos_u';
    ...
    SET @query = CONCAT('
			UPDATE  ic_fac_tr_inventario_boletos
				SET '
					,lo_id_sucursal
					,lo_consolidado
					,lo_fecha
