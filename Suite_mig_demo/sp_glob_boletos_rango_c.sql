DELIMITER $$
CREATE DEFINER=`suite_deve`@`%` PROCEDURE `sp_glob_boletos_rango_c`(
...
BEGIN
/*
	@nombre:		sp_glob_boletos_rango_c
	@fecha: 		29/08/2016
...
*/
    ...
    BEGIN
        SET pr_message = 'ERROR store sp_glob_boletos_rango_c';
	END ;

    SET lo_bol_inicial = (SELECT
							 COUNT(*)
						  FROM ic_glob_tr_inventario_boletos
						  WHERE pr_bol_inicial BETWEEN bol_inicial AND bol_final
						  AND id_proveedor = pr_id_proveedor
						  AND id_grupo_empresa = pr_id_grupo_empresa);

    SET lo_bol_final = (SELECT
					    	 COUNT(*)
					     FROM ic_glob_tr_inventario_boletos
					     WHERE pr_bol_final BETWEEN bol_inicial AND bol_final
					     AND id_proveedor = pr_id_proveedor
					     AND  id_grupo_empresa = pr_id_grupo_empresa);
