DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `sp_rep_anticipos_cfdi_importes_pendientes_c`(
...
BEGIN
/*
	@nombre:		sp_rep_anticipos_cfdi_importes_pendientes_c
	@fecha:			03/05/2019
...
*/
    ...
    DECLARE lo_moneda				VARCHAR(200) DEFAULT '';

	DECLARE EXIT HANDLER FOR SQLEXCEPTION
	BEGIN
		SET pr_message = 'ERROR store sp_rep_anticipos_cfdi_importes_pendientes_c';
	END ;
							...
							SELECT
								CASE
									WHEN ',pr_id_moneda,' = 149 THEN
										SUM((ant.anticipo_usd - ant.importe_aplicado_usd))
									WHEN ',pr_id_moneda,' = 49 THEN
										SUM((ant.anticipo_eur - ant.importe_aplicado_eur))
									ELSE
										SUM(ant.anticipo_moneda_base - ant.importe_aplicado_base)
								END importe_ant_pendientes
