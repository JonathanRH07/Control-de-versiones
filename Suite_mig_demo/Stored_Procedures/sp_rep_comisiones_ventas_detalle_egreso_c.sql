DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `sp_rep_comisiones_ventas_detalle_egreso_c`(
...
*/
    ...
    DROP TABLE IF EXISTS tmp_report_comisiones_detalle_emision_egre;
    DROP TABLE IF EXISTS tmp_report_comisiones_vencimiento_egre;
    DROP TABLE IF EXISTS tmp_report_comisiones_detalle_pago_egre;
	...
	IF pr_tipo_fecha > 0 THEN
		...
		IF pr_tipo_fecha = 1 THEN
			...
			SET @queryemisionegre = CONCAT('CREATE TEMPORARY TABLE tmp_report_comisiones_detalle_emision_egre
											SELECT
												fac.id_factura,
												ser.cve_serie serie,
			...
			-- SELECT @queryemisionegre;
            PREPARE stmt FROM @queryemisionegre;
			EXECUTE stmt;
			...
			FROM tmp_report_comisiones_detalle_emision_egre;

		/* TIPO DE FECHA 'VENCIMIENTO' */
		ELSEIF pr_tipo_fecha = 2 THEN

			SET @queryvencimientoegre = CONCAT('CREATE TEMPORARY TABLE tmp_report_comisiones_vencimiento_egre
												SELECT
													fac.id_factura,
													ser.cve_serie serie,
													fac.fac_numero fac_nc,
			...
			-- SELECT @queryvencimientoegre;
            PREPARE stmt FROM @queryvencimientoegre;
			EXECUTE stmt;
			DEALLOCATE PREPARE stmt;
			...
			SELECT
				...
				IFNULL(cuota_fija,0) cuota_fija,
				IFNULL(comision_neta,0) comision_neta
			FROM tmp_report_comisiones_vencimiento_egre;

        /* TIPO DE FECHA 'PAGO' */
		ELSEIF  pr_tipo_fecha = 3 THEN

			SET @querypagoegre = CONCAT('CREATE TEMPORARY TABLE tmp_report_comisiones_detalle_pago_egre
										SELECT
											fac.id_factura,
											ser.cve_serie serie,
											fac.fac_numero fac_nc,
            ...
            -- SELECT @querypagoegre;
            PREPARE stmt FROM @querypagoegre;
			EXECUTE stmt;
			DEALLOCATE PREPARE stmt;
			...
			SELECT
				...
				IFNULL(cuota_fija,0) cuota_fija,
				IFNULL(comision_neta,0) comision_neta
			FROM tmp_report_comisiones_detalle_pago_egre;
