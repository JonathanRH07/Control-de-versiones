DELIMITER $$
CREATE DEFINER=`suite_deve`@`%` PROCEDURE `sp_gds_cxs_f`(
...
*/
    ...
    BEGIN
        SET pr_message = 'ERROR store sp_gds_cxs_f';
	END ;
    ...
    IF pr_desc_producto != '' THEN
		SET lo_desc_producto = CONCAT(' AND IF(cxs.id_producto IS NULL AND cxs.automatico = "S","Todos",pro.descripcion) LIKE "%', pr_desc_producto , '%" ');
	END IF;
    ...
    IF ( pr_consulta_gral !='' ) THEN
		SET lo_consulta_gral = CONCAT(' AND (fp.cve_forma_pago LIKE "%'	, pr_consulta_gral, '%"
                                        ...
                                        OR  IF(cxs.id_producto IS NULL AND cxs.automatico = "S","Todos",pro.descripcion) LIKE "%'	, pr_consulta_gral, '%"
                                        OR  cxs.importe LIKE "%'	, pr_consulta_gral, '%"
                                        OR  IF(cxs.en_otra_serie="S","Otra serie","Misma serie") LIKE "%', pr_consulta_gral, '%"
    ...
    IF pr_id_cliente = 0 THEN
		...
		SET @query = CONCAT('SELECT
								...
								cxs.alcance,
								pro.id_producto,
                                IF(cxs.id_producto IS NULL AND cxs.automatico = "S","Todos",pro.descripcion) desc_producto,
								prv.cve_proveedor,
								ser.cve_serie,
								fp.cve_forma_pago,
								cxs.fecha_mod fecha_mod,
							...
							INNER JOIN suite_mig_conf.st_adm_tr_usuario usuario
								ON usuario.id_usuario=cxs.id_usuario
							WHERE cxs.id_grupo_empresa = ? ',
								lo_automatico,
								lo_referencia,
								lo_desc_producto,
								lo_alcance,
	...
	ELSE
		SET @query = CONCAT('SELECT
								...
								IFNULL(cli.id_cxs_xcliente, 0) id_cxs_xcliente,
								0 as seleccionado,
                                cxs.fecha_mod fecha_mod,
								concat(usuario.nombre_usuario," ",
								usuario.paterno_usuario) usuario_mod
							...
							INNER JOIN ic_glob_tr_forma_pago fp ON
								cxs.id_forma_pago = fp.id_forma_pago
							LEFT JOIN ic_gds_tr_cxs_xcliente cli ON
		...
		EXECUTE stmt USING @id_cliente, @id_grupo_empresa, @ini, @fin;
    END IF;
