DELIMITER $$
CREATE DEFINER=`suite_deve`@`%` PROCEDURE `sp_gds_remarks_grupo_emp_u`(
	IN  pr_id_remarks_grupo_emp	INT,
...
*/
	...
	SET @query = CONCAT('UPDATE ic_gds_tr_remarks_grupo_emp
							SET ',
								lo_valor_remark,
								lo_obligatorio,
								lo_id_stat,
								...
								' , fecha_mod  = sysdate()
							WHERE id_remarks_grupo_emp = ?
                            AND   id_grupo_empresa     = ',pr_id_grupo_empresa);

	PREPARE stmt
	FROM @query;

	SET @id_remarks_grupo_emp = pr_id_remarks_grupo_emp;
	EXECUTE stmt USING @id_remarks_grupo_emp;
