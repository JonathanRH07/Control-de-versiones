...
*/
    ...
    BEGIN
        SET pr_message = 'ERROR store sp_rep_ventas_cliente_x_mes_detalle_egre_c';
	END ;
	...
	SELECT
		...
		total.concepto ,
		total.cve_proveedor,
		IFNULL(total.tarifa_facturada + IFNULL(total.importe_markup,0) - IFNULL(isr.isr,0),0) tarifa_facturada,
		IFNULL(iva.iva,0) iva,
		IFNULL(tua.tua,0) tua,
		IFNULL(otros.otros,0) otros,
		IFNULL((IFNULL(total.tarifa_facturada,0) + IFNULL(iva.iva,0) + IFNULL(tua.tua,0) + IFNULL(otros.otros,0) + IFNULL(total.importe_markup,0)) - IFNULL(isr.isr,0),0) total_servicio,
		total.boleto
	...
	LEFT JOIN tmp_detalles_egreso_isr isr ON
