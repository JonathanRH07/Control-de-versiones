DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `sp_rep_acumulados_sucursal_mes_u`(
	 IN  pr_fecha							VARCHAR(7),
     OUT pr_message							VARCHAR(500)
)
BEGIN
/*
    @nombre:		sp_rep_acumulados_sucursal_mes_u
	@fecha:			2019/02/22
...
*/
    ...
    DECLARE cu_sucursal CURSOR FOR
	SELECT
    ...
    BEGIN
        SET pr_message = 'ERROR store sp_rep_acumulados_sucursal_mes_u';
        ROLLBACK;
	END ;
    ...
    DROP TEMPORARY TABLE IF EXISTS tmp_factura_acumulado_sucursal;

    OPEN cu_sucursal;

		loop_obtidsucursal: LOOP

           FETCH cu_sucursal INTO lo_id_sucursal;

           IF @hecho THEN
				LEAVE loop_obtidsucursal;
		   END IF;
           ...
           SET @queryacumulado = CONCAT('CREATE TEMPORARY TABLE tmp_factura_acumulado_sucursal
											SELECT
			...
			INTO
                ...
                lo_acumulado_usd,
                lo_acumulado_eur
			FROM tmp_factura_acumulado_sucursal;
