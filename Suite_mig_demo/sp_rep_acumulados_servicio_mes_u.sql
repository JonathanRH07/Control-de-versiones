DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `sp_rep_acumulados_servicio_mes_u`(
	 IN  pr_fecha							VARCHAR(7),
	 OUT pr_message							VARCHAR(500)
)
BEGIN
/*
    @nombre:		sp_rep_acumulados_servicio_mes_u
	@fecha:			2019/02/22
...
*/
    ...
    DECLARE lo_acu_id_servicio				INT;
    ...
    DECLARE cu_servicio CURSOR FOR
	SELECT
    ...
    BEGIN
        SET pr_message = 'ERROR store sp_rep_acumulados_servicio_mes_u';
        ROLLBACK;
	END ;
    ...
    DROP TEMPORARY TABLE IF EXISTS tmp_factura_acumulado_servicio;

    OPEN cu_servicio;

		loop_obtidservicio: LOOP

           FETCH cu_servicio INTO lo_id_servicio;

           IF @hecho THEN
				LEAVE loop_obtidservicio;
		   END IF;
            ...
            /* -------------------------------------------------------- */
			 ...
			 SET @queryacumulado = CONCAT('CREATE TEMPORARY TABLE tmp_factura_acumulado_servicio
											SELECT
												id_servicio,
                                            ...
                                            AND fecha <= ''',pr_fecha,'''
											AND id_servicio = ',lo_id_servicio,'
											ORDER BY fecha ASC');
			...
			INTO
                ...
                lo_acumulado_usd,
                lo_acumulado_eur
			FROM tmp_factura_acumulado_servicio;
