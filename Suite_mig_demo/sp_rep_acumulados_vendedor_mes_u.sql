DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `sp_rep_acumulados_vendedor_mes_u`(
	 IN  pr_fecha							VARCHAR(7),
	 OUT pr_message							VARCHAR(500)
)
BEGIN
/*
    @nombre:		sp_rep_acumulados_vendedor_mes_u
	@fecha:			2019/02/22
...
*/
    ...
    DECLARE lo_acu_id_vendedor				INT;
    ...
    DECLARE cu_vendedor CURSOR FOR
	SELECT
    ...
    BEGIN
        SET pr_message = 'ERROR store sp_rep_acumulados_vendedor_mes_u';
        ROLLBACK;
	END ;
    ...
    DROP TEMPORARY TABLE IF EXISTS tmp_factura_acumulado_vendedor;

    OPEN cu_vendedor;

		loop_obtidvendedor: LOOP

           FETCH cu_vendedor INTO lo_id_vendedor;

           IF @hecho THEN
				LEAVE loop_obtidvendedor;
		   END IF;
		   ...
		   DROP TEMPORARY TABLE IF EXISTS tmp_factura_acumulado_vendedor;
            ...
            SET @queryacumulado = CONCAT('CREATE TEMPORARY TABLE tmp_factura_acumulado_vendedor
											SELECT
												id_vendedor,
                                            ...
                                            AND fecha <= ''',pr_fecha,'''
											AND id_vendedor = ',lo_id_vendedor,'
											ORDER BY fecha ASC');
			...
			INTO
                ...
                lo_acumulado_usd,
                lo_acumulado_eur
			FROM tmp_factura_acumulado_vendedor;
           ...
           ELSE
				...
				WHERE fecha = pr_fecha
				AND id_vendedor = lo_acu_id_vendedor
				AND acumulado_moneda_base = 0
                AND acumulado_usd = 0
                AND acumulado_eur = 0;
           END IF;
