DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `sp_rep_comisiones_ced_x_serv_c`(
...
BEGIN
/*
	@nombre:		sp_rep_comisiones_ced_x_serv_c
	@fecha:			10/12/2018
...
*/
	...
	BEGIN
		SET pr_message = 'ERROR store sp_rep_comisiones_ced_x_serv_c';
	END;
    ...
    IF pr_tipo_fecha > 0 THEN
		...
		IF pr_tipo_fecha = 1 THEN
            ...
            SET @query = CONCAT('SELECT
									...
									SUM(det.comision_titular) comision_vendedor
								...
								JOIN ic_cat_tr_plan_comision plan ON
									vend.id_comision = plan.id_plan_comision
		...
		ELSEIF pr_tipo_fecha = 2 THEN
			...
			SET @query = CONCAT('SELECT
									plan.descripcion,
									SUM(fac_det.comision_agencia) comision_agencia,
									SUM(fac_det.comision_titular) comision_vendedor
								FROM ic_glob_tr_cxc cxc
								JOIN ic_glob_tr_cxc_detalle cxc_det ON
									cxc.id_cxc = cxc_det.id_cxc
								JOIN ic_cat_tr_vendedor vend ON
									cxc.id_vendedor = vend.id_vendedor
								JOIN ic_cat_tr_plan_comision plan ON
									vend.id_comision = plan.id_plan_comision
		...
		ELSEIF  pr_tipo_fecha = 3 THEN
			...
			SET @query = CONCAT('SELECT
								...
								JOIN ic_fac_tr_pagos_detalle pag_det ON
									pag.id_pago = pag_det.id_pago
								JOIN ic_glob_tr_cxc cxc ON
									pag_det.id_cxc = cxc.id_cxc
								JOIN ic_glob_tr_cxc_detalle cxc_det ON
									cxc.id_cxc = cxc_det.id_cxc
								JOIN ic_cat_tr_vendedor vend ON
									cxc.id_vendedor = vend.id_vendedor
