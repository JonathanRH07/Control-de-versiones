DELIMITER $$
CREATE DEFINER=`suite_deve`@`%` PROCEDURE `sp_fac_factura_cfdi_u`(
...
BEGIN
	...
	*/
    ...
    DECLARE  lo_archivo_addenda				TEXT DEFAULT '';
	...
	END IF;
    /*
    IF pr_factura_pdf != '' THEN
		-- SET lo_factura_pdf = CONCAT(' factura_pdf=  ', pr_factura_pdf, ',');
        SET lo_factura_pdf = CONCAT(' factura_pdf=  ');
	ELSE
        SET pr_factura_pdf = '';
	END IF;
    */
 ...
 --  Select @query;
    ...
    IF pr_factura_pdf != '' THEN
		UPDATE ic_fac_tr_factura_cfdi
		SET factura_pdf = pr_factura_pdf
			-- factura_xml = lo_factura_xml
		WHERE id_cfdi = pr_id_cfdi;
    END IF;
